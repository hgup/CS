...
pdf_file <- "pdf/projectplanning.pdf"
...
library(readxl)
mySchedule <- read_excel("datafiles/projectplanning.xlsx")
n <- nrow(mySchedule)
...
for (i in 1:n)
{
  ...
  else if (!(is.na(what[i])) & nchar(as.character (what[i]))>0){
    x1<-as.Date.POSIXct(as.numeric(mySchedule[i,'from' ]))
    x2<-as.Date.POSIXct(as.numeric(mySchedule[i,'to' ]))
    x3<-x1+(as.numeric(x2-x1)*as.numeric(mySchedule[i,'done' ]/100))
    x<-c(x1, x2)
    x_done <- c(x1,x3)
    y <- c(i,i)
    segments(myBegin, i , myEnd, i , col = 'grey')
    lines(x,y,lwd = 20, col = myColour[i])
    points(myEnd + 90, i, cex = as.numeric(mySchedule[i,'Persons']*mySchedule[i,'Durance'])**0.5,
  ...
  {
    x4<-as.Date.POSIXct(as.numeric(mySchedule[i,'AG_from' ]))
    x5<-as.Date.POSIXct(as.numeric(mySchedule[i,'AG_to' ]))
    x_ag <- c(x4,x5)
  ...
  {
    x4<-as.Date.POSIXct(as.numeric(mySchedule[i,'when' ]))
    myHalf <- (myEnd-myBegin)/2
    if (x3 - x1 < myHalf)
  {
    points(x4 + 5, i, pch = 18 , cex = 1.25, col = "red")
    text(x4-5,i,Milestone[i], adj=1, xpd=T, cex=0.7)
  }
  }
}
axis(3, at=c(myBegin, myEnd), labels=c( format (myBegin, format="%d/%m/%Y"),
format (myEnd, format="%d/%m/%Y")))
...
mtext(paste("planning status: ",format(myToday, format="%d/%m/%y"),sep=""),
